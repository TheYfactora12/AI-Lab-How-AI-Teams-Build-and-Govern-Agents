...
This is the click-by-click route for presenting the completed certificate experiment. Keep this page open as your private guide. The current evidence is contract 1.2; files labeled historical explain earlier work and should not be used for current result counts.
...
| evaluation_snapshots/contract-1.2/ | Exact saved outputs and receipt | Backup evidence; JSON is hard to present live |
...
| 4 | [Current comparison](https://github.com/TheYfactora12/AI-Lab-How-AI-Teams-Build-and-Govern-Agents/blob/main/CURRENT_COMPARISON.md) | Actual contract 1.2 results, limitations and direct W&B links |
| 5 | [Current V1 in Weave](https://wandb.ai/kevinmedeiros-masterclass/ai-lab-agent-governance/weave/calls/01a06d7e-b22d-7497-a050-15150061f01b) | C03’s V1 scope error and exact failure |
| 6 | [Current V2 in Weave](https://wandb.ai/kevinmedeiros-masterclass/ai-lab-agent-governance/weave/calls/01a06d7f-274c-7f26-8385-6c4dabf64fbd) | C03’s rejected scope decisions and withheld assessment |
...
| 8 | [Successful workflow](https://github.com/TheYfactora12/AI-Lab-How-AI-Teams-Build-and-Govern-Agents/actions/runs/33901467965) | Tests and both hosted evaluations executed successfully |
...
The Weave links are private project links. They can work for you while returning 404 to someone without access. The committed [receipt](evaluation_snapshots/contract-1.2/receipt.json) preserves their identifiers and the executed source revision.
...
Do not call the 22 regression tests or 23 offline probes additional model cases. There are five live cases per version.

### 4. Current comparison — 40 seconds

Point to **What matters** and the C03 row.

Say: “Both versions received three automated passes and two blocks, with zero application errors. The equal total does not mean nothing changed; case-level inspection matters.”
...
Show the two rejected requirements, `needs_clarification`, `packet_status: withheld`, and the clarification questions.

Say: “V2 adds a validation gate after generation. It catches both unsupported scope decisions, preserves the uncertainty and sends the issue to a person.”
...
Close with: “V2 caught the targeted unknown-scope error, but readiness-state and judge limitations remain. I would keep the system in draft-only use until those issues are tested under a new frozen contract.”
...
- “The judge is ground truth.” C02 and C05 show remaining interpretation problems.
...
The repository includes a repeatable [presentation link audit](presentation_snapshots/link-check.json). It verifies local targets, maps GitHub file links to tracked paths, and matches the private Weave URLs and Actions run to the successful contract 1.2 receipt. It cannot prove that a different browser is signed in to the private W&B project.
...
- [ ] You can explain why equal overall counts do not erase the case-level improvement.
- [ ] You can name the C02 readiness and C05 judge limitations.
